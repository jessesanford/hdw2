...
```bash
curl -L https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/hdw2.sh -o hdw2.sh && chmod +x ./hdw2.sh && ./hdw2.sh
```
> **Note:** This script is now named `hdw2.sh`; older instructions may reference `bypass-mdm.sh`. Replace `YOUR_USERNAME/YOUR_REPO` with the repository that hosts `hdw2.sh` (for this repo, use `jessesanford/hdw2`). If any step fails, fix the error and re-run the command. This fork preserves the original author credit in the script header.
